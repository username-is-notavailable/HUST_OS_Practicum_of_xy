...
ssize_t sys_user_exit(uint64 code) {
  ...
  if(parent&&parent->status==BLOCKED&&(parent->wait_for_pid==current->pid||parent->wait_for_pid==-1)){
    parent->status=READY;
...
//
// kerenl entry point of wait. added @lab3_ch1
//
extern process procs[NPROC];
uint64 sys_user_wait(uint64 pid) {
  if((0<pid&&pid<NPROC&&procs[pid].status!=FREE&&procs[pid].status!=ZOMBIE&&procs[pid].parent==current)||pid==-1) {
