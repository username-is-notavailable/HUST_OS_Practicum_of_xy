  ...
  : htif(htif), memif(&htif->memif()), table(2048)
{
  ...
  table[2045] = &syscall_t::sys_get_stdin_buffer;
  table[2046] = &syscall_t::sys_readdir;
  table[2047] = &syscall_t::sys_wait_for_a_key;
...
reg_t syscall_t::sys_wait_for_a_key(reg_t path, reg_t a1, reg_t a2, reg_t a3, reg_t a4, reg_t a5, reg_t a6)
{
  if(system("stty -icanon"))fprintf(stderr,"Error when set 'stty -icanon'");  //无缓冲
  if(system("stty -echo"))fprintf(stderr,"Error when set 'stty -echo'");    //无回显

  char a;
  if(readkey>0||kbhit()){
    a=getchar();
    if(readkey>0)readkey--;
    if(a==27){readkey=2;} 
  }
  else a=0;
  if(system("stty -icanon"))fprintf(stderr,"Error when set 'stty icanon'");  //恢复
  if(system("stty -echo"))fprintf(stderr,"Error when set 'stty echo'");
  reg_t ret = a;
  return ret;
...
reg_t syscall_t::sys_readdir(reg_t pname, reg_t len, reg_t dname, reg_t offset, reg_t type, reg_t a5, reg_t a6)
{
  std::vector<char> name(len);
  memif->read(pname,len,name.data());
  DIR *dir=opendir(do_chroot(name.data()).c_str());
  dirent *d=NULL;
  int ret=-1;
  if(!dir)return sysret_errno(-1);
  for(reg_t i=0;i<=offset;i++){
    d=readdir(dir);
    if(!d)break;
    if(!strcmp(d->d_name,".")||!strcmp(d->d_name,".."))i--;
  }
  if(d){
    ret=0;
    memif->write(dname,strlen(d->d_name)+1,d->d_name);
    memif->write(type, sizeof(char),&d->d_type);
    // fprintf(stderr,"%d\n",d->d_type);
  }
  return ret;
}

reg_t syscall_t::sys_get_stdin_buffer(reg_t stdin_buff, reg_t max_len, reg_t a2, reg_t a3, reg_t a4, reg_t a5, reg_t a6){
  char buf[max_len]={0};
  int len=0, oflag=-1, nflag=-1;
  oflag = fcntl(STDIN_FILENO,F_GETFL);
  nflag = oflag | O_NONBLOCK;
  if(system("stty -icanon"))fprintf(stderr,"Error when set 'stty -icanon'");  //无缓冲
  if(system("stty -echo"))fprintf(stderr,"Error when set 'stty -echo'");    //无回显
  fcntl(STDIN_FILENO,F_SETFL,nflag);
  // fprintf(stderr,"!!!!!!\n");
  if((len=read(STDIN_FILENO,buf,max_len))>0)memif->write(stdin_buff,len+1,buf);
    // fprintf(stderr, "error when read stdin with nonblock\n");
  fcntl(STDIN_FILENO,F_SETFL,oflag);
  if(system("stty -icanon"))fprintf(stderr,"Error when set 'stty icanon'");  //恢复
  if(system("stty -echo"))fprintf(stderr,"Error when set 'stty echo'");
  return sysret_errno(len);
}
